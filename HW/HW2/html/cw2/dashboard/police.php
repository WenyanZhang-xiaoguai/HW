...
if (isPostRequest()) {
    ...
    if (!$username || !$password) {
        echo '<script>alert("Please fill in all fields");window.location.href="police.php"</script>';
        exit();
    }
    if ($db->get_row("SELECT * from `officers` where Username = '$username'")) {
        echo '<script>alert("Officer already exists");window.location.href="police.php"</script>';
        exit();
    }
    $sql = "INSERT INTO `officers` (Username, Password) VALUES ('$username', '$password')";
    $db->update($sql);
    logger('add officer');
    echo '<script>alert("Officer added successfully");window.location.href="police.php"</script>';
    exit();
}
